...
export default function FlashcardSession({
  ...
  subjectId   = "",
  subjectName = "",
  // ✅ isReview: true = جاي من إشعار مراجعة → يبدأ من startIndex مباشرة
  //             false = جلسة عادية → يستأنف من التقدم المحفوظ
  isReview    = false,
  startIndex  = 0,
}) {
  ...
  // ─── تحديد نقطة البداية ───────────────────────────────────────────────────
  // إذا isReview=true  → ابدأ من startIndex (البطاقة الصعبة المحددة)
  // إذا isReview=false → ابدأ من التقدم المحفوظ في localStorage
  const getInitialIndex = () => {
    if (isReview) {
      // جاي من إشعار مراجعة — ابدأ من البطاقة المحددة مباشرة
      return startIndex >= total ? 0 : startIndex;
    }
    // جلسة عادية — استأنف من التقدم المحفوظ
    const savedIndex = user?.id ? getProgress(user.id, sessionKey) : 0;
    return savedIndex >= total ? 0 : savedIndex;
  };
  ...
  // رسالة الاستئناف: تظهر فقط في الجلسة العادية إذا في تقدم محفوظ
  const [showResume,   setShowResume]   = useState(!isReview && initialIndex > 0);
  ...
  // ─── حفظ التقدم — فقط في الجلسة العادية وليس المراجعة ───────────────────
  useEffect(() => {
    if (!user?.id || finished || isReview) return;
    saveProgress(user.id, sessionKey, currentIndex);
  }, [currentIndex, user?.id, sessionKey, finished, isReview]);
  ...
  const handleRating = (rating) => {
    ...
    if (currentIndex + 1 >= total) {
      if (user?.id && !isReview) clearProgress(user.id, sessionKey);
      setFinished(true);
    } else {
      setFlipped(false);
  ...
  const restart = () => {
    if (user?.id && !isReview) clearProgress(user.id, sessionKey);
    setCurrentIndex(isReview ? startIndex : 0);
    setFlipped(false);
    setResults([]);
    setFinished(false);
    setShowResume(false);
      ...
      >
        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-8 text-center">
          ...
          <h2 className="text-2xl font-black text-gray-800 dark:text-white mb-2">
            {isReview ? "انتهت المراجعة! 🧠" : "انتهت الجلسة! 🎉"}
          </h2>
          <p className="text-gray-500 mb-6">لقد أجبت على {results.length} سؤال</p>

          {hard > 0 && (
            <div className="mb-5 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-2xl p-3 text-sm text-orange-700 dark:text-orange-400 font-bold flex items-center gap-2">
              🧠 {hard} سؤال صعب — ستصلك تذكيرات مراجعة بعد 3 أيام
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3 mb-8">
            {[
              ...
              <div key={r.label}
                ...
                <p className={`text-2xl font-black text-${r.color}-600 dark:text-${r.color}-400`}>{r.count}</p>
                <p className={`text-sm font-bold text-${r.color}-600 dark:text-${r.color}-400`}>{r.label}</p>
              </div>
            ))}
          </div>
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        {/* ✅ شريط المراجعة — يظهر فقط لما isReview=true */}
        {isReview && (
          <div className="flex items-center gap-2 px-5 py-2.5 bg-violet-500/10 border-b border-violet-500/20 text-violet-600 dark:text-violet-400 text-sm font-bold">
            🧠 وضع المراجعة — السؤال الصعب
          </div>
        )}

        {/* رسالة الاستئناف — تظهر فقط في الجلسة العادية */}
        <AnimatePresence>
          {showResume && (
            <motion.div
        ...
        <div className="p-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {subjectName && (
              <span className="text-xs text-gray-400 font-medium">· {subjectName}</span>
            )}
          </div>
            ...
            {!flipped ? (
              ...
              <>
                <p className="text-center text-xs text-gray-400">
                  🧠 اختيار <span className="font-bold text-orange-500">صعب</span> سيُذكّرك بهذا السؤال بعد 3 أيام
                </p>
